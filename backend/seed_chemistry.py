"""
seed_chemistry.py
Seeds Chemistry Grade 8 into the study_buddy DB (NGSS MS-PS1: Matter and Its Interactions).
Generates detailed lessons and mixed chemistry questions via Claude Sonnet.

Run from the backend folder:
    python seed_chemistry.py
...
CURRICULUM = [
    {
        ...
        "description": "Chemistry — Atoms, Reactions, Solutions & the Periodic Table (NGSS MS-PS1)",
        "units": [
            {
                "name": "Atomic Structure & Elements",
                "term": 1,
                "topics": [
                    "Atoms: Structure & Subatomic Particles",
                    "The Periodic Table: Organization & Trends",
                    "Elements, Compounds & Mixtures",
                    "Chemical Symbols & Formulas",
                ],
            },
            {
                "name": "States & Properties of Matter",
                "term": 1,
                "topics": [
...
LESSON_PROMPT = """\
You are writing a chemistry textbook chapter for a {grade}th grader in a US public \
school (Massachusetts, NGSS curriculum — Matter and Its Interactions, MS-PS1).
...
knows or experiences daily (cooking, sports drinks, batteries, fireworks, cleaning \
products, the human body). Give historical context or a surprising real-world \
...
the human body, everyday objects)
...
- Where helpful, include a simple worked example (e.g. a balanced equation, a \
labeled diagram described in words, a sample calculation)
...
example from medicine, food science, technology, or the environment.
...
would happen if..." or "Imagine you are a chemist and..."
...
Tone: Enthusiastic, clear, conversational — like a great chemistry teacher who loves \
...
QUESTIONS_PROMPT = """\
Generate exactly 6 multiple-choice questions for a {grade}th grade chemistry lesson \
on "{topic}" (unit: {unit}, NGSS MS-PS1).
...
correct use of a key chemistry term directly tied to "{topic}".
...
Q3 — APPLICATION/SCENARIO: Describe an original everyday or lab scenario (2-4 \
sentences) and ask the student to apply what they learned about "{topic}" to explain \
or predict what happens.

Q4 — REAL-WORLD CONNECTION: Ask about a real-world example, technology, or natural \
phenomenon (medicine, food, environment, industry) that relates to "{topic}".

Q5 — PROCESS/CALCULATION OR CLASSIFICATION: If "{topic}" involves a process, sequence, \
calculation, or classification (e.g. balancing an equation, classifying a reaction \
type, identifying states of matter, comparing physical/chemical changes), test that \
skill directly. If not applicable, ask a question that requires comparing two related \
...
- Grade-appropriate difficulty for a {grade}th grader following NGSS MS-PS1 standards
...
print(f"\nStudy Buddy — Chemistry Curriculum Seeder")
print(f"Model  : {MODEL}")
...
for subj_data in CURRICULUM:
    ...
    if existing_subj:
        ...
        print(f"\nSubject Chemistry Grade {grade} already exists (id={subject.id}) — adding missing topics only")
    else:
        ...
        db.commit()
        db.refresh(subject)
        print(f"\nCreated subject: Chemistry Grade {grade} (id={subject.id})")
